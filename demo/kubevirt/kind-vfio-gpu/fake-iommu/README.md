...
[`../fake-pci/`](../fake-pci/), which in turn lets a KubeVirt VMI
configured for DRA PCI passthrough reach `Running`.
...
synthetic `0xfaca:` PCI bus, that call returns NULL and `vfio-pci`
refuses to bind. KubeVirt's virt-handler then fails its pre-start hook,
and the VMI never moves out of `Scheduling`.
...
| Guest OS sees a non-crashing PCI device | depends on `fake-pci` BAR backing |
| Guest OS sees a *working* device | **no** (no real hardware behind the BARs) |
...
- `fake-pci` module (either built and ready to load, or already
...
```bash
cd demo/clusters/kind-vfio-gpu/fake-iommu
...
sudo insmod ../fake-pci/fake-pci.ko
...
# 3. vfio-pci can now bind (using the synthetic vendor/device IDs)
echo e1a5 d0c5 | sudo tee /sys/bus/pci/drivers/vfio-pci/new_id
...
|---|---|---|
| `target_domain` | `0xfaca` | PCI domain whose devices to claim. Must match `fake-pci`'s `pci_domain`. |
...
<later, when fake-pci adds new devices>
...
succeeds. virt-handler proceeds. virt-launcher hands the group fd to
QEMU. QEMU constructs the device. The VM reaches `Running`.

## Files

| File | License | Notes |
|---|---|---|
| `fake-iommu.c` | GPL-2.0-only | Kernel module |
