...
func TestTestFeatureGateSetFromMap(t *testing.T) {
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fg := newTestFeatureGates(version.MajorMinor(1, 36))
			require.NoError(t, fg.SetFromMap(tt.enabled))
			for feature, want := range tt.want {
				assert.Equal(t, want, fg.Enabled(feature), "feature %s", feature)
			}
		})
	}
}
